...
class OperationTest < ActiveSupport::TestCase
  ...
  test 'saving and gathering' do
    new_operation = Operation.new(amount: 5, odate: '2023-01-05', description: 'new description',
                                  category_id: Category.find_by(name: 'First cat').id, otype: 0,
                                  user_id: User.find_by(username: 'First').id)
    new_operation.save
  ...
  test 'saving and updating' do
    new_operation = Operation.new(amount: 10, odate: '2023-01-10', description: 'new description',
                                  category_id: Category.find_by(name: 'First cat').id, otype: 0,
                                  user_id: User.find_by(username: 'First').id)
    new_operation.save
  ...
  test 'saving and destroying' do
    new_operation = Operation.new(amount: 15, odate: '2023-01-15', description: 'new description',
                                  category_id: Category.find_by(name: 'First cat').id, otype: 0,
                                  user_id: User.find_by(username: 'First').id)
    new_operation.save
  ...
  test 'return false if amount is missed' do
    new_operation = Operation.new(odate: '2023-01-11', description: 'some description',
                                  category_id: Category.find_by(name: 'First cat').id, otype: 1,
                                  user_id: User.find_by(username: 'First').id)
    assert_not(new_operation.valid?)
  end

  test 'return false if odate missed' do
    new_operation = Operation.new(amount: 5, description: 'some description',
                                  category_id: Category.find_by(name: 'First cat').id, otype: 1,
                                  user_id: User.find_by(username: 'First').id)
    assert_not(new_operation.valid?)
  end
  ...
  test 'return true if description missed' do
    new_operation = Operation.new(amount: 5, odate: '2023-01-11', category_id: Category.find_by(name: 'First cat').id,
                                  otype: 1, user_id: User.find_by(username: 'First').id)
    assert(new_operation.valid?)
  end

  test 'return false if category_id missed' do
    new_operation = Operation.new(amount: 5, odate: '2023-01-11', description: 'some description', otype: 1,
                                  user_id: User.find_by(username: 'First').id)
    assert_not(new_operation.valid?)
  end

  test 'return false if otype missed' do
    new_operation = Operation.new(amount: 5, odate: '2023-01-11', description: 'some description',
                                  category_id: Category.find_by(name: 'First cat').id,
                                  user_id: User.find_by(username: 'First').id)
    assert_not(new_operation.valid?)
  end
  ...
  test 'check reports_data_by_dates with fix_1 and fix_2' do
    graph_data = Operation.reports_data_by_dates('2023-01-10', '2023-02-12', '1',
                                                 Category.find_by(name: 'First cat').id,
                                                 User.find_by(username: 'First').id)
    assert_equal([['2023-01-11', 9.99], ['2023-02-11', 99.99]], graph_data.map do |e|
      [e[0].strftime('%Y-%m-%d'), e[1].to_f]
    end)
  end

  test 'check reports_data_by_dates with empty dates period' do
    graph_data = Operation.reports_data_by_dates('2020-09-01', '2020-10-01', '1',
                                                 Category.find_by(name: 'Third cat').id,
                                                 User.find_by(username: 'Second').id)
    assert_empty(graph_data)
  end

  test 'check reports_data_by_dates with dates period from faker' do
    graph_data = Operation.reports_data_by_dates('2022-11-01', '2023-01-01', '1',
                                                 Category.find_by(name: 'Third cat').id,
                                                 User.find_by(username: 'Second').id)
  ...
  test 'check reports_data_by_dates with empty category' do
    graph_data = Operation.reports_data_by_dates('2022-11-01', '2023-01-01', '0',
                                                 Category.find_by(name: 'Twelfth cat').id,
                                                 User.find_by(username: 'First').id)
    assert_empty(graph_data)
  end
  ...
  test 'check reports_data_by_dates with wrong pair otype-category_id' do
    graph_data = Operation.reports_data_by_dates('2022-11-01', '2023-01-01', '1',
                                                 Category.find_by(name: 'Tenth cat').id,
                                                 User.find_by(username: 'First').id)
    assert_empty(graph_data)
  end
  ...
  test 'check reports_data_by_category with fix_1 and fix_2' do
    diagram_data = Operation.reports_data_by_category('2023-01-10', '2023-02-12', '1',
                                                      Category.find_by(name: 'First cat').id,
                                                      User.find_by(username: 'First').id)
  ...
  test 'check reports_data_by_category with false dates period' do
    diagram_data = Operation.reports_data_by_category('2020-09-01', '2020-10-01', '1',
                                                      Category.find_by(name: 'Third cat').id,
                                                      User.find_by(username: 'Second').id)
  ...
  test 'check reports_data_by_category with true dates period from faker' do
    diagram_data = Operation.reports_data_by_category('2022-11-01', '2023-01-01', '1',
                                                      Category.find_by(name: 'Third cat').id,
                                                      User.find_by(username: 'Second').id)
  ...
  test 'check reports_data_by_category with empty category' do
    diagram_data = Operation.reports_data_by_category('2022-11-01', '2023-01-01', '0',
                                                      Category.find_by(name: 'Twelfth cat').id,
                                                      User.find_by(username: 'First').id)
    assert_empty(diagram_data)
  end
